...
def comp_res_hpwl(placedb, node_pos, ratio):
    """HPWL half of the original ``comp_res`` metric, without Prim cost.

    This is deliberately kept line-for-line equivalent to the HPWL
    accumulation in ``comp_res`` so it can be recorded every epoch without
    paying for the unrelated MST calculation.
    """
    ...
    for net_name in placedb.net_info:
        ...
        for node_name in placedb.net_info[net_name]["nodes"]:
            ...
            pin_x = (
                node_pos[node_name][0] * ratio
                + h / 2.0
                + placedb.net_info[net_name]["nodes"][node_name]["x_offset"]
            )
            pin_y = (
                node_pos[node_name][1] * ratio
                + w / 2.0
                + placedb.net_info[net_name]["nodes"][node_name]["y_offset"]
            )
        ...
        for port_name in placedb.net_info[net_name]["ports"]:
            pin_x = placedb.port_info[port_name]["x"]
            pin_y = placedb.port_info[port_name]["y"]
            max_x = max(pin_x, max_x)
            min_x = min(pin_x, min_x)
            max_y = max(pin_y, max_y)
            min_y = min(pin_y, min_y)
        hpwl_tmp = (max_x - min_x) + (max_y - min_y) if min_x <= placedb.max_height else 0
